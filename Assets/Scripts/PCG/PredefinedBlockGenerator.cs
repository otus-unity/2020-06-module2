...
public class PredefinedBlockGenerator : AbstractGenerator
{
    ...
    static List<PredefinedBlock> GetBlocksMatchingMask(PredefinedBlock[] blocks, int requirePresent, int requireAbsent)
    {
    ...
    protected override void GeneratorImpl()
    {
        ...
        PredefinedBlock startBlock = GetRandomBlockMatchingMask(blocks, 0, GetRequireAbsentMask(new Vector2Int(startX, startY)));
        ...
        do {
            ...
            if ((bitmask & PredefinedBlock.BitmaskLeft) != 0 && !bitmasks.ContainsKey(new Vector2Int(pos.x - 1, pos.y))) {
                stack.Push(new Vector2Int(pos.x - 1, pos.y));
                SpawnNode(new Vector2Int(pos.x - 1, pos.y));
            }

            if ((bitmask & PredefinedBlock.BitmaskRight) != 0 && !bitmasks.ContainsKey(new Vector2Int(pos.x + 1, pos.y))) {
                stack.Push(new Vector2Int(pos.x + 1, pos.y));
                SpawnNode(new Vector2Int(pos.x + 1, pos.y));
            }

            if ((bitmask & PredefinedBlock.BitmaskDown) != 0 && !bitmasks.ContainsKey(new Vector2Int(pos.x, pos.y - 1))) {
                stack.Push(new Vector2Int(pos.x, pos.y - 1));
                SpawnNode(new Vector2Int(pos.x, pos.y - 1));
            }

            if ((bitmask & PredefinedBlock.BitmaskUp) != 0 && !bitmasks.ContainsKey(new Vector2Int(pos.x, pos.y + 1))) {
                stack.Push(new Vector2Int(pos.x, pos.y + 1));
                SpawnNode(new Vector2Int(pos.x, pos.y + 1));
            }
        } while (stack.Count > 0);
